...
|---|---|---|---|---|---|
| 1 | **RIO 전원 켜짐, 아무도 없음** — 부팅 직후 빈 방 | sleepy | NormalFace(dim) | `sleepy.png` + dim overlay (alpha 30%) | `face_compositor._apply_dim()` |
...
**state_gui.py 검증:**
```
Context=Away + Activity=Idle    → Mood=sleepy, UI=NormalFace(dim) 확인
...
F6 → sleep/wake 시나리오 자동 재생
  0s:   calm     (평온)
  1s:   sleepy   (반감긴 눈... 졸려...)
  3s:   sleepy + dim (완전 어두움, 잠듦)
  6s:   welcome  (어! 왔어? 반가워!)
  7.5s: calm     (다시 평온)

관찰:
  - sleepy에서 dim으로 어두워지는 과정
...
state_demo.py에서 s (시나리오 자동 재생):
 ...
 1. 부팅 완료     → 빈 방, 어두운 대기      → sleepy, NormalFace(dim)

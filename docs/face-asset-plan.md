...
| **inactive** | 10 (dimmed) | sleepy와 같은 표정 파일, 밝기 30% | Context=Away, Activity=Idle |
...
`inactive` (Away+Idle)는 별도 에셋 없이 런타임에서 처리:
- `FaceCompositor._MOOD_ALIASES`가 `"inactive" → "sleepy"`로 자동 매핑하여 `sleepy.png`을 그림
- 밝기(opacity)는 caller(`DisplayRenderer` 등)가 `set_opacity(0.3)`으로 제어
- 결과적으로 `inactive` = `sleepy` 표정 + alpha 30% 어두운 오버레이
...
1. **full-frame 8종** 추출: calm, attentive, sleepy, alert, startled, confused, welcome, happy
   (9번째 mood인 `inactive`는 sleepy.png + dim 처리로 별도 에셋 불필요)
